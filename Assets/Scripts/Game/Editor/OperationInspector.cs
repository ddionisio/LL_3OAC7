...
public class OperationInspector : PropertyDrawer {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        ...
        var rect = new Rect(position.position, new Vector2(40f, position.height));
		...
		switch((OperatorType)propOp.enumValueIndex) {
            case OperatorType.Multiply:
				rect.x += 44f;
				eq = propOp1.intValue * propOp2.intValue;
				EditorGUI.LabelField(rect, " = " + eq);
				break;
